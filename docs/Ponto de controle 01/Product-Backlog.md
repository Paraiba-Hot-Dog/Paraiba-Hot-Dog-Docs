...
| **RQ11** | O sistema deve permitir que o administrador sobreponha o horário padrão para datas festivas. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ13** | O sistema deve listar os produtos com nome, composição detalhada dos ingredientes e preço em destaque. |
| **RQ14** | O sistema deve permitir a navegação entre categorias (Ex: Prensados, Bebidas, Combos) através de um menu de abas ou scroll lateral. |
| **RQ15** | O sistema deve exibir fotos reais dos produtos ao lado da descrição para facilitar a identificação visual. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ16** | O sistema deve exibir o horário de funcionamento e o status atual (Ex: Aberto agora) diretamente no card da unidade. |
| **RQ17** | O sistema deve integrar um mapa interativo que permite o uso de gestos (zoom e arrastar) para explorar as localizações. |
| **RQ18** | O sistema deve fornecer um link direto ("Open in Maps") que redireciona o usuário para o aplicativo de navegação externo. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ19** | O sistema deve fornecer uma grade de produtos com fotos para seleção rápida por toque. |
| **RQ20** | O sistema deve permitir a inserção de observações personalizadas por item. |
| **RQ21** | O sistema deve calcular automaticamente o subtotal, taxas, descontos e o valor total do pedido. |
| **RQ22** | O sistema deve permitir selecionar múltiplos métodos de pagamento (Pix, Cartão, Dinheiro). |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ23** | O sistema deve exigir permissão de administrador e/ou caixa para autorizar cancelamentos. |
| **RQ24** | O sistema deve obrigar a seleção de um motivo para cada pedido cancelado. |
| **RQ25** | O sistema deve registrar logs técnicos de mensagens de erro ocorridas durante a venda. |
| **RQ26** | O sistema deve gerar um relatório mensal de "Perdas por Cancelamento". |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ27** | O sistema deve permitir o cadastro/identificação de clientes via número de telefone no checkin/checkout. |
| **RQ28** | O sistema deve calcular e acumular ponto(s) automaticamente com base no valor da compra. |
| **RQ29** | O sistema deve exibir o saldo de ponto(s) disponível(eis) para resgate. |
| **RQ30** | O sistema deve registrar uma data de validade para os pontos acumulados. |
| **RQ31** | O sistema deve permitir criar campanhas de promoção com título, descrição e mídia. |
| **RQ32** | O sistema deve enviar notificações via WhatsApp e/ou Push notification. |
...
| **RQ34** | O sistema deve calcular o lucro líquido ($Vendas - Custos$). |
...
| **RQ39** | O sistema deve identificar o "Produto Mais Lucrativo" (maior margem). |
...
| **RQ44** | O sistema deve permitir a comparação de desempenho entre diferentes períodos. |
...
| **RQ48** | O sistema deve gerar alertas para produtos com baixa venda (itens parados). |
| **RQ49** | O sistema deve exibir um dashboard com resumo de vendas do dia. |
| **RQ50** | O sistema deve listar os produtos mais vendidos no período atual de visualização. |
| **RQ51** | O sistema deve mostrar o ticket médio de vendas especificamente do dia atual. |
| **RQ52** | O sistema deve permitir filtrar dados por período (hora, turno ou personalizado). |
...
| **RQ54** | O sistema deve separar permissões entre perfis de "Caixa" e "Administrador". |
| **RQ55** | O sistema deve registrar logs de auditoria (quem alterou preços ou deletou pedidos). |
| **RQ56** | O sistema deve realizar o logoff automático após "X" minutos de inatividade na área administrativa. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ57** | O sistema deve permitir criar produtos, preços e fotos via painel administrativo. |
| **RQ58** | O sistema deve permitir editar produtos, preços e fotos via painel administrativo. |
| **RQ59** | O sistema deve permitir remover itens do cardápio com confirmação obrigatória. |
| **RQ60** | O sistema deve permitir a alteração de banners da página inicial para campanhas. |
| **RQ61** | O sistema deve permitir o reajuste de preços em massa (por porcentagem). |
| **RQ62** | O sistema deve exibir a data da última atualização de cada item do cardápio. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ63** | O sistema deve permitir a exportação de relatórios financeiros exclusivamente no formato PDF. |
| **RQ64** | O sistema deve permitir a geração de relatórios baseada em datas personalizadas. |
| **RQ65** | O sistema deve disparar um envio de resumo de faturamento diário automático por e-mail no fechamento. |
| **RQ66** | O sistema deve destacar a margem de lucro por categoria no relatório gerado. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ67** | O sistema deve permitir o cadastro de novas unidades com endereço e coordenadas. |
| **RQ68** | O sistema deve permitir editar horários de funcionamento e status de operação por unidade. |
| **RQ69** | O sistema deve disponibilizar os dados de geolocalização via API para consumo da interface. |
| **RQ70** | O sistema deve permitir desativar temporariamente uma unidade para ocultação no mapa. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ71** | O sistema deve exibir pedidos em "cards" com itens, observações e tempo decorrido. |
| **RQ72** | O sistema deve mudar a cor do card (Semaforização) conforme o tempo de preparo. |
| **RQ73** | O sistema deve permitir dar "baixa" no item ou pedido completo via toque ou atalho. |
| **RQ74** | O sistema deve exibir o histórico dos pedidos por dia. |
...
| ID | Descrição do Requisito Funcional |
| :--- | :--- |
| **RQ75** | O sistema deve exibir uma interface de status otimizada para Smart TVs. |
| **RQ76** | O sistema deve emitir um alerta sonoro padrão quando um pedido for marcado como "Pronto". |
| **RQ77** | O sistema deve remover o pedido da lista de "Pronto" após tempo configurável. |
